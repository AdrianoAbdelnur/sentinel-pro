...
import { badRequest, forbidden, projectGlobalSyncRun } from "@/app/api/internal/catalog/v2/delivery";
...
export async function GET(request: Request, { params }: Context) {
  ...
  return Response.json({ status: { ...result.status, latestRun: projectGlobalSyncRun(result.status.latestRun) } });
}
