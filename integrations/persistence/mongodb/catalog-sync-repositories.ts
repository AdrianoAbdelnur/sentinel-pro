...
function toDocument(run: CatalogSyncRun, existing?: CatalogSyncRunDocument): CatalogSyncRunDocument {
  return {
    schemaVersion: 1,
    id: run.id,
...
export function createCatalogSyncRepositories(db: Db, session?: ClientSession) {
  ...
  const syncRuns = {
      async findLatest(connectionId: string) { return (await runs.find({ connectionId }, options(session)).sort({ startedAt: -1, attempt: -1 }).limit(1).next()) as CatalogSyncRun | undefined; },
      async findLastSuccess(connectionId: string) { return (await runs.find({ connectionId, status: "succeeded" }, options(session)).sort({ completedAt: -1 }).limit(1).next()) as CatalogSyncRun | undefined; },
      async findLastConfirmed(connectionId: string) { return (await runs.find({ connectionId, status: "succeeded", "snapshot.status": "complete" }, options(session)).sort({ completedAt: -1 }).limit(1).next()) as CatalogSyncRun | undefined; },
      async claimActive(run: CatalogSyncRun) { try { await runs.insertOne(toDocument(run), options(session)); return "claimed" as const; } catch (error) { if (isDuplicate(error)) return "already-active" as const; throw error; } },
      async save(run: CatalogSyncRun) { const existing = await findRun({ id: run.id }); await runs.replaceOne({ id: run.id }, toDocument(run, existing ?? undefined), { upsert: true, ...options(session) }); },
    };
  const syncLeases = {
      async claim(connectionId: string, runId: string, claimNow: Date, durationMs: number) {
        ...
        try {
          const before = await leases.findOneAndUpdate({ connectionId, $or: [{ leaseUntil: { $lte: claimNow } }, { runId }] }, { $set: { runId, leaseUntil, updatedAt: claimNow }, $setOnInsert: { schemaVersion: 1, createdAt: claimNow } }, { upsert: true, returnDocument: "before", ...options(session) });
