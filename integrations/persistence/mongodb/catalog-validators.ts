...
export const catalogValidators: Record<string, Document> = {
  catalog_vehicles: schema(["id", "placementFleetId"], { id: text, normalizedPlate: { bsonType: "string" }, plate: { bsonType: "string" }, placementFleetId: text, name: text, make: text, model: text, company: text, active: { bsonType: "bool" }, placement: { bsonType: "object", required: ["groupId", "authority", "assignedAt"], properties: { groupId: text, authority: { enum: ["authoritative", "fallback"] }, evidenceBindingId: text, assignedAt: date } } }),
  ...
  provider_contributions: schema(["id", "connectionId", "externalId", "vehicleId", "capabilities", "presence"], { id: text, connectionId: text, externalId: text, vehicleId: text, deviceId: text, observedCompany: text, observedPlate: text, observedName: text, observedMake: text, observedModel: text, observedAt: date, capabilities: { bsonType: "object", additionalProperties: status }, presence: { enum: ["present", "absent"] } }),
  catalog_devices: schema(["id", "vehicleId", "connectionId", "deviceId", "capabilities", "presence"], { id: text, vehicleId: text, connectionId: text, deviceId: text, kind: text, make: text, model: text, status: { enum: ["active", "inactive"] }, capabilities: { bsonType: "object", additionalProperties: status }, presence: { enum: ["present", "absent"] } }),
  provider_vehicle_observations: schema(["id", "contributionId", "connectionId", "deviceId", "companyResolution", "observedAt"], { id: text, contributionId: text, connectionId: text, deviceId: text, providerKey: text, plate: text, normalizedPlate: text, name: text, make: text, model: text, company: text, directFleetId: text, companySourceFleetId: text, companyResolution: { enum: ["direct", "ancestor", "unresolved", "not-applicable"] }, presence: { enum: ["present", "absent"] }, active: { bsonType: "bool" }, observedAt: date }),
  catalog_conflicts: schema(["id", "vehicleId", "kind", "values", "status"], { id: text, vehicleId: text, kind: { enum: ["company", "identity"] }, values: { bsonType: "array", items: text }, status: { enum: ["open", "resolved"] }, canonicalValue: text }),
