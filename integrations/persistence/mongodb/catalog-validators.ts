...
export const catalogValidators: Record<string, Document> = {
  ...
  external_fleet_identities: { $jsonSchema: { bsonType: "object", additionalProperties: false, required: ["_id", "schemaVersion", "id", "organizationId", "connectionId", "entityKind", "externalId", "label", "createdAt", "updatedAt"], properties: { _id: objectId, schemaVersion: version, id: text, organizationId: text, connectionId: text, entityKind: { enum: ["fleet"] }, externalId: text, label: text, fleetId: text, createdAt: date, updatedAt: date } } },
  external_vehicle_identities: { $jsonSchema: { bsonType: "object", additionalProperties: false, required: ["_id", "schemaVersion", "id", "organizationId", "connectionId", "entityKind", "externalId", "createdAt", "updatedAt"], properties: { _id: objectId, schemaVersion: version, id: text, organizationId: text, connectionId: text, entityKind: { enum: ["vehicle"] }, externalId: text, vehicleId: text, lastSeenRunId: text, presence: { enum: ["present", "absent"] }, createdAt: date, updatedAt: date } } },
};
