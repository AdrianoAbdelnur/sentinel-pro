...
export function createMongoCatalogRepositories(db: Db, session?: ClientSession): MongoCatalogRepositories {
  ...
  return {
    ...
    syncRuns: {
      ...
      async findLastConfirmed(organizationId, connectionId) { const document = await syncRunsCollection.find({ organizationId, connectionId, status: "succeeded", fullSnapshot: true, "snapshot.status": "complete" }, options(session)).sort({ completedAt: -1 }).limit(1).next(); return document ? toCatalogSyncRunDomain(document) : undefined; },
