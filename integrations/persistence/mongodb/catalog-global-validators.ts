...
export const catalogGlobalValidators: Record<string, Document> = {
  ...
  catalog_runs_v2: schema(["id", "connectionId", "status", "trigger", "startedAt", "counts", "snapshot"], { id: text, connectionId: text, status: { enum: ["active", "succeeded", "failed"] }, trigger: { enum: ["initial", "manual", "internal", "scheduler"] }, startedAt: date, completedAt: date, checkpoint: text, counts: { bsonType: "object" }, snapshot: { bsonType: "object" }, failure: { bsonType: "object" } }),
