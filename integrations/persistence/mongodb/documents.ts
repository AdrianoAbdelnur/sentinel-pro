...
export type UserDocument = {
  _id?: ObjectId; schemaVersion: number; id: string; firstName: string; lastName: string; emailNormalized: string; passwordHash: string; passwordChangeRequired: boolean; status: "active" | "inactive"; failureCount: number; failureWindowStartedAt?: Date; blockedUntil?: Date; authorizationVersion: number; platformRole?: "super-admin"; createdAt: Date; updatedAt: Date;
};
...
export function toUserDomain({ id, firstName, lastName, emailNormalized, passwordHash, passwordChangeRequired, status, failureCount, blockedUntil, authorizationVersion, platformRole, createdAt, updatedAt }: UserDocument): IdentityUser { return { id, firstName, lastName, emailNormalized, passwordHash, passwordChangeRequired, status, failureCount, ...(blockedUntil ? { blockedUntil } : {}), authorizationVersion, ...(platformRole ? { platformRole } : {}), createdAt, updatedAt }; }
