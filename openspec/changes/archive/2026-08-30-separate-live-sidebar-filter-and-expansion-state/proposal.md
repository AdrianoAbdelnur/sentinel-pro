...
- [ ] After page initialization, an operator-controlled fleet opens or closes only after its direct toggle.
- [ ] Filter changes preserve an operator’s expansion choices.
- [ ] Existing selection/count behavior remains intact.

## Later Compatibility

The later server-pagination capability initializes groups returned on the
active page as expanded. That page-lifecycle behavior supersedes the original
"all fleets start collapsed" assumption but does not change this fix's durable
invariant: provider, status, and search filter changes never mutate expansion.
