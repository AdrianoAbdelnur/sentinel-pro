...
- [ ] 5.2b **GREEN/REFACTOR:** Wire Howen to automatic cron synchronization — add the per-connection company-assignment domain field (or equivalent), its Mongo document/validator/migration, and cron composition wiring in `app/api/internal/catalog/synchronize/composition.ts` so Howen connections resolve to a real `CatalogImportSource` instead of `unsupported-provider` [`catalog-synchronization`, `howen-catalog-import`]. Closes carried-forward risk #16. **NOT COMPLETE — groundwork only, risk #16 stays OPEN.** PR (`feat/catalog-howen-cron`, base `feat/catalog-admin-api-status`) ships `ProviderConnection.companyId` (optional field), the Mongo document/validator/migration (idempotent, backward-compatible, mutation-proven), and the cron composition wiring (`missing-company-assignment` outcome, distinguishable from `unsupported-provider`/`not-found`) — all independently verified correct by a fresh-context review. But that review found the field is a dead end: **no authorized write path exists to set `connection.companyId`** — `ProviderConnectionRepository.save` is a bare unauthorized `replaceOne` with zero production call sites in `app/`, so no real Howen connection can ever be assigned a Company and every one will permanently report `missing-company-assignment`. Task 5.2b's own stated purpose (wiring Howen to *automatic* sync) is therefore not achieved. Also found: (a) the import path trusts `candidate.companyId` with no tenant check (`belongsToOrganization`/`requireCompanyInTenant` exist but are never called on this path — Cybermapa is safe only because `bindProviderCompany` pre-validates upstream; Howen has no such gate), a latent cross-tenant risk once a write path is added; (b) the manual sync route (`app/api/admin/catalog/connections/[connectionId]/sync/route.ts`) returns the same "unsupported provider" Spanish message for both a genuinely unsupported provider and a correctly-configured-but-unassigned Howen connection; (c) `createHowenConnectionFactory` buckets a broken `HOWEN_BASE_URL`/config into the same `missing-company-assignment` outcome as a truly missing assignment. Follow-up PR `feat/catalog-connection-assignment` (from `feat/catalog-howen-cron`) must add the authorized assignment use case (mirroring `bindProviderCompany`'s admin/tenant/`belongsToOrganization` checks, full four-cell matrix), an admin route for it, defense-in-depth tenant verification on the import path with a cross-tenant test, and separate operator-facing outcomes for "needs assignment" vs "provider misconfigured" — only then does risk #16 close and this task mark `[x]`.
