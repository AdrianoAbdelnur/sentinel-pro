...
The system MUST determine sidebar visibility from provider, status, and search filters. It MUST determine each fleet's expanded state solely from explicit operator fleet-toggle actions. A filter change MUST NOT open, close, or otherwise alter any fleet's expansion state.

#### Scenario: All fleets start collapsed
- GIVEN the operator has not toggled any fleet
- WHEN the live sidebar renders without filters
- THEN every visible fleet is collapsed
- AND no vehicle rows are shown
...
#### Scenario: Only a fleet toggle changes expansion
- GIVEN any combination of provider, status, and search filters
- WHEN the operator does not toggle a fleet
- THEN no fleet expansion state changes
- AND a direct fleet toggle is the only action that may change that state
